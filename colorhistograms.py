...
def load_img(f):
    ...
    for i in range(len(lines)):
        ...
        if im1 is None:
            ...
            hist_r = cv2.calcHist([im1], [0], None, [256], [0, 256]).flatten()
            hist_g = cv2.calcHist([im1], [1], None, [256], [0, 256]).flatten()
            hist_b = cv2.calcHist([im1], [2], None, [256], [0, 256]).flatten()
            color_hist = np.concatenate([hist_r, hist_g, hist_b])
          # vec = np.reshape(im1, [-1])
            imgs.append(color_hist)
